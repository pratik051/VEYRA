...
export function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 3.4 Marketplace Sources Tab */}
      {activeTab === "Marketplace Sources" && (
        <div className="space-y-6">
          <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-3">
              <div>
                <h3 className="text-base font-black text-slate-900">Supported Indian Marketplace Channels</h3>
                <p className="text-xs text-slate-400">Direct sourcing connections for Amazon IN, Flipkart, Myntra, boAt &amp; Tata CLiQ</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                ...
                >
                  {auditingAll ? "Verifying..." : "⚡ Run Live URL Audit"}
                </button>
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition cursor-pointer"
                >
                  {syncingProvider ? "Syncing..." : "🔄 Sync All Channels"}
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {marketplaceProviders.map((provider) => (
                <div
                  key={provider.id}
                  className="rounded-2xl border border-slate-200 p-5 space-y-3 bg-slate-50/50"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-black text-slate-900 text-sm">{provider.displayName}</span>
                    <button
                  ...
                  <div className="text-xs text-slate-500 space-y-1">
                    <p>Imported Products: <strong className="text-slate-800">{provider.importedCount}</strong></p>
                    <p>Last Synced: <strong className="text-slate-800">{provider.lastSyncedAt ? new Date(provider.lastSyncedAt).toLocaleTimeString() : "Never"}</strong></p>
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-200 flex gap-2">
                    <button
                      ...
                      className="flex-1 py-1.5 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 font-bold text-xs transition cursor-pointer"
                    >
                      {syncingProvider === provider.id ? "Syncing..." : "Sync Feed"}
                    </button>
                  </div>
                </div>
              ))}

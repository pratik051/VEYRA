...
export default async function MarketplaceChannelPage({
  params,
  searchParams
}: {
  ...
  const sourceId = params.source?.toLowerCase().trim();
  const meta = getMarketplaceMeta(sourceId);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-10 py-8">
      ...
      <nav className="flex items-center gap-2 text-xs font-semibold text-neutral-500">
        ...
        <span>/</span>
        <Link href="/shop" className="hover:text-neutral-900 transition">Marketplaces</Link>
        <span>/</span>
      ...
      <div className="rounded-3xl bg-gradient-to-br from-neutral-950 via-neutral-900 to-neutral-950 text-white p-6 sm:p-10 lg:p-12 relative overflow-hidden shadow-xl border border-neutral-800">
        ...
        <div className="relative z-10 max-w-3xl space-y-5">
          ...
          <p className="text-sm sm:text-base text-neutral-300 leading-relaxed font-medium">
            {meta.tagline}. Order any product from {meta.domain} with transparent INR ➔ NPR conversion and direct door-to-door delivery across Nepal.
          </p>
      ...
      <div className="space-y-3">
        <span className="text-xs font-black uppercase tracking-wider text-neutral-400">Switch Marketplace Channel</span>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
            {sections.map((s) => {
              ...
              return (
                <Link
                  key={s.id}
                  href={`/marketplace/${meta.id}?section=${s.id}&sort=${sortBy}`}
      ...
      <div className="space-y-3">
        <span className="text-xs font-black uppercase tracking-wider text-neutral-400">
          Paste Any {meta.name} Link for Instant Quote
        </span>
        <LinkVerifier />
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-200 pb-4">
          <div>
            ...
            <p className="text-xs text-neutral-500 mt-1">
              Showing authentic products sourced exclusively from {meta.name} ({meta.domain}).
            </p>
          </div>
        </div>

        {products.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-neutral-300 bg-white p-12 text-center space-y-4">
            <div className="flex justify-center">
              <MarketplaceLogo marketplace={meta.id} className="h-10 w-auto opacity-70" />
            </div>
            <h3 className="text-lg font-bold text-neutral-900">
              No pre-cataloged products for {meta.name} in this section
            </h3>
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-neutral-950 px-5 py-2.5 text-xs font-bold text-white hover:bg-red-600 transition"
            >
